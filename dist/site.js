    ...
    */
		...
		value: function scoreNode(node, otherNodes) {
			...
			for (var i = 0; i < lengthSites; i++) {
				...
				score += new _levenshtein2.default(text, otherText).distance;
			}
			var entropy = Math.floor(score * text.length);
			site.$(node).attr('entropy', entropy);

			var childEntropy = entropy;
			_underscore2.default.forEach(node.children(), function (child, index) {
				childEntropy += _this2.scoreNode(site.$(child), otherNodes.map(function (element, i) {
